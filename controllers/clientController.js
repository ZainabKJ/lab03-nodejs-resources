...
const loginControl = (request, response) => {
  ...
  if (!username || !password) {
      ...
      if (request.session && request.session.user) {
        response.render("log", { link: 'Already Logged in' });
          ...
          clientServices.loginService(username, password, function(err, dberr, client) {
              ...
              if (client === null) {
                  ...
                  response.render("log", { link: `Login failed` });
              } else {
                  ...
                  response.render("log", { link:`Login for ${username} with ID.${client[0].num_client} was successful!` });
  ...
  clientServices.registerService(client, function(err, exists, insertedID) {
      console.log("User from register service :" + insertedID);
       if(exists){
          ...
          response.render("reg", { link: `registration failed. Username ${username} already taken!` ,});
      } else {
          client.num_client = insertedID;
          console.log(`Registration ${username}, ${insertedID} successful!`);
          response.render("reg", { link: `Successful registration ${username}. Your ID is ${insertedID} !`,});
      }
      response.end();
  });
};
